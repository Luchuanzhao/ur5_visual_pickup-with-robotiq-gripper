...
class ur5_mp:
    def __init__(self):
        ...
        print("======start_pose0=",start_pose)#初始状态为趴着的状态
        ...
        start_pose111 = self.arm.get_current_pose(self.end_effector_link).pose
        print("=======start_pose1=",start_pose111)#检测物块状态

        # Specify end states (drop object)
        self.end_joint_states = deepcopy(self.default_joint_states)
        self.end_joint_states[0] = -3.65#用于确定箱子正上方坐标，放置物体时使用
    ...
    def tracking_callback(self, msg):
        ...
        #why waypoints[0].position.x is between -0.6 to 0.6???
        ...
        if self.track_flag:
            ...
            if len(self.pointx)>3:
                if len(self.pointx)==4:
                    ...
                    #wpose.position.x += -4.2 * abs(x_speed)
                    ...
                    if len(self.pointx)==5:
                        ...
                        #self.grasp_joint_states = self.arm.get_current_joint_values()
                        #self.grasp_joint_states[5] = 1.57691
                        #self.arm.set_joint_value_target(self.grasp_joint_states)
                        #self.arm.go()
                        self.gripper.set_joint_value_target([0.35])
                        self.gripper.go()
                        print("======gripper is closed")#抓取物体
                    ...
                    if len(self.pointx)<6:
                        ...
                        if tracker.flag2:
                            self.track_flag=False
                        #start_pose1 = self.arm.get_current_pose(self.end_effector_link).pose
                        ...
                        transition_pose.position.z = 0.1 + self.object_cnt*0.025 #放置时的坐标
                        self.waypoints.append(deepcopy(transition_pose))
                        ...
                        tracker.flag2 = 0
                        self.cxy_pub.publish(tracker)
                        self.gripper.set_joint_value_target([0.001])
                        self.gripper.go()
            ...
            # Set the next waypoint to the right 0.5 meters
                ...
                wpose.position.y += self.error_y*0.03/150#往右移动跟随物体
                wpose.position.z = 0.25  #第一次跟随物体时的高度
                #wpose.position.z = 0.4005
                
                start_pose111 = self.arm.get_current_pose(self.end_effector_link).pose
                print("=======following object",start_pose111)
        		

            if self.phase == 1:#第一阶段，就是还没有往箱子里放置物体的阶段
                self.waypoints.append(deepcopy(wpose))


                self.pointx.append(wpose.position.x)
                self.pointy.append(wpose.position.y)#如果还在这个阶段，就逐渐往右移动，point.x+1
                ...
                if 1-fraction < 0.2:
                    rospy.loginfo("Path computed successfully. Moving the arm111.")
            ...
            wpose.orientation.x = 0.0029
            wpose.orientation.y = 0.9998
            wpose.orientation.z = -0.0152
            wpose.orientation.w = 0.0001 #跟随失败后复位位姿

            #wpose.orientation.x = 0.4811
            #wpose.orientation.y = 0.4994
            #wpose.orientation.z = 0.5121
            #wpose.orientation.w = 0.5069 #跟随失败后复位位姿
